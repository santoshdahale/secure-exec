...
The host and sandbox processes communicate over a **Unix domain socket** using length-prefixed MessagePack framing. The channel is a trust boundary with the following properties:
...
- **FD table limit**: Each isolate can hold up to **1024 open file descriptors**. Attempts to open beyond this limit are rejected with `EMFILE`. This prevents sandbox code from exhausting host-level FD resources.
